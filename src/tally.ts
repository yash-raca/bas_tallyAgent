...
import { database } from './database'; // Capitalized because we created this class earlier
import { 
    tallyConfig, 
    tableConfigYAML, 
    companyInfo, 
...
class _tally {
 ...
 private exportReports(): Promise<void> {
        return new Promise<void>(async (resolve, reject) => {
            logger.logMessage('\nExtracting Financial Reports (Balance Sheet, P&L)...');
            
            // ✅ We explicitly define a DIFFERENT target table for each report
            const reportsToFetch = [
                { 
                    dbName: 'Balance Sheet', 
                    tableName: 'rpt_balance_sheet',    
                    tallyIds: ['Balance Sheet'],
                    extraVars: '' 
                },
                { 
                    dbName: 'Profit & Loss', 
                    tableName: 'rpt_profit_and_loss',  
                    tallyIds: ['Profit &amp; Loss A/c', 'Profit &amp; Loss', 'Profit and Loss'],
                    extraVars: '<ISVERTICAL>No</ISVERTICAL>' 
                },
                { 
                    dbName: 'Cash Flow', 
                    tableName: 'rpt_cash_flow',  
                    tallyIds: ['Cash Flow'],
                    extraVars: '' 
                },
                { 
                    dbName: 'Fund Flow', 
                    tableName: 'rpt_funds_flow',  
                    tallyIds: ['Funds Flow', 'Fund Flow'],
                    extraVars: '' 
                },
                { 
                    dbName: 'Trial Balance', 
                    tableName: 'rpt_trial_balance',  
                    tallyIds: ['Trial Balance'],
                    extraVars: '<EXPLODEFLAG>Yes</EXPLODEFLAG>' 
                },
                { 
                    dbName: 'Stock Summary', 
                    tableName: 'rpt_stock_summary',  
                    tallyIds: ['Stock Summary'],
                    extraVars: '<EXPLODEFLAG>Yes</EXPLODEFLAG>' 
                },
                { 
                    dbName: 'Ratio Analysis', 
                    tableName: 'rpt_ratio_analysis',  
                    tallyIds: ['Ratio Analysis'],
                    extraVars: '' 
                },
                { 
                    dbName: 'Bills Receivable', 
                    tableName: 'rpt_bills_receivable',  
                    tallyIds: ['Bills Receivable'],
                    extraVars: '<EXPLODEFLAG>Yes</EXPLODEFLAG><ISBILLWISEON>Yes</ISBILLWISEON>' 
                },
                // 🚀 NEW: Exact Bill-by-Bill Payables
                { 
                    dbName: 'Bills Payable', 
                    tableName: 'rpt_bills_payable',  
                    tallyIds: ['Bills Payable'],
                    extraVars: '<EXPLODEFLAG>Yes</EXPLODEFLAG><ISBILLWISEON>Yes</ISBILLWISEON>' 
                }
            ...
            for (const report of reportsToFetch) {
                try {
                    let dateFromStr = utility.Date.format(this.periodFromDate || new Date(), 'd-MMM-yyyy');
                    let dateToStr = utility.Date.format(this.periodToDate || new Date(), 'd-MMM-yyyy');
                    let companyTag = this.config.company ? utility.String.escapeHTML(this.config.company) : '##SVCurrentCompany';
                    
                    let xmlContent = '';
                    let success = false;

                    for (const tallyId of report.tallyIds) {
                        logger.logMessage(`  -> Fetching ${report.dbName} (Attempt: ${tallyId.replace('&amp;', '&')})`);
                        
                        let xmlPayload = `<?xml version="1.0" encoding="utf-8"?><ENVELOPE><HEADER><VERSION>1</VERSION><TALLYREQUEST>Export</TALLYREQUEST><TYPE>Data</TYPE><ID>${tallyId}</ID></HEADER><BODY><DESC><STATICVARIABLES><SVEXPORTFORMAT>$$SysName:XML</SVEXPORTFORMAT><SVFROMDATE>${dateFromStr}</SVFROMDATE><SVTODATE>${dateToStr}</SVTODATE><SVCURRENTCOMPANY>${companyTag}</SVCURRENTCOMPANY><EXPLODEFLAG>Yes</EXPLODEFLAG>${report.extraVars}</STATICVARIABLES></DESC></BODY></ENVELOPE>`;

                        xmlContent = await this.postTallyXML(xmlPayload);
                        
                        if (!xmlContent.includes('Could not find Report') && !xmlContent.includes('Unknown Request') && xmlContent.trim() !== '') {
                            success = true;
                            break; 
                        }
                    }
                    ...
                    if (!success) {
                        logger.logMessage(`  ⚠️ Tally rejected the request for ${report.dbName}. Skipping.`);
                        continue;
                    }
                    ...
                    try {
                        ...
                        logger.logMessage(`  ⚠️ Failed to parse XML for ${report.dbName}.`);
                        continue;
                    }
                    ...
                    if (jsonObj && jsonObj.ENVELOPE) {
                        ...
                        // ✅ Saves immediately to its specific table. They will no longer overwrite each other!
                        await database.bulkLoadTableJson({ name: report.tableName }, payload);
                        logger.logMessage(`  ✅ Successfully saved ${report.dbName} to the ${report.tableName} table.`);
                    }

                } catch (e) {
                    logger.logError(`tally.exportReports() - ${report.dbName}`, e);
                }
            }
